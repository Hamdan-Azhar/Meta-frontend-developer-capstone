...
const ReserveTablePage = () =>{
    const [occasionOptions,setOccasionOptions] = useState(["birthday","anniversary","business meeting"]);
    ...
    const handleSubmit = async () => {
      try {
        const reservationFormData = {
          ...
          numberOfGuests:noOfGuestsOptions[0]
        }

        await axios.post("http://localhost:8000/reservation/add-reservation",reservationFormData)
    ...
    const fetchData = async (date) => {
      try {
        ...
        const response = await axios.get("http://localhost:8000/reservation/available-times",{
          params : { date: formattedDate }
        });
        setReservationTimes([...response.data.times])
      } catch (error) {
    ...
    return(
     ...
     <section id={styles["ReserveTable"]}>
      <div>
        ...
        <img src={restaraunt_image} alt="restaurant"/>
      </div>
     </section>
     <Footer/>
     </>
